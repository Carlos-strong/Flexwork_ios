...
const FALLBACK_USER: DashboardUser = { initials: "MN", name: "Manœuvre", role: "Manœuvre", avatarGradient: "from-[#008751] to-[#FCD116]" };

export default function ManoeuvreDashboard({ initialNav = "dashboard" }: { initialNav?: string } = {}) {
  const { data: session, status } = useSession() as { data: Session | null; status: string };
  const router = useRouter();
  const nav = initialNav;
  const goTo = (id: string) => router.push(providerUrl("manoeuvre", id));
  ...
  return (
    <DashboardLayout mode="prestataire" user={user} navItems={providerNav("manoeuvre")} activeNav={nav} onNavChange={goTo} badgeCounts={badgeCounts}
      title="Tableau de bord" solde="32 000"
    >
      {nav === "messages" ? (
        <Messagerie currentUserId={currentUserId} />
      ) : nav === "missions" ? (
        <MissionsSection role="manoeuvre" />
      ) : nav === "candidatures" ? (
        <CandidaturesSection />
      ) : nav === "offres" ? (
        <OffresSection />
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-6">
        <div><h1 className="text-[22px] md:text-[26px] font-bold text-[#0A1931] tracking-tight">Bonjour {identity?.name ?? "Manœuvre"} 👋</h1><p className="text-[13px] text-zinc-500 mt-1"><span className="text-[#FF7A00]">✦</span> <span className="font-semibold text-[#FF7A00]">2 missions</span> en cours • 3 nouvelles disponibles</p></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-[20px] border border-gray-100 overflow-hidden"><div className="px-5 py-4 border-b border-gray-50"><h3 className="font-semibold text-[14px]">📸 Pointage GPS</h3></div><div className="px-5 py-4 space-y-2 text-[12px]">
            ...
            {/* « Requis » uniquement si l'Admin KYC a activé l'exigence sur ce compte
                (User.garantRequired, défaut OFF) — sinon le garant reste une option. */}
            {identity?.garantRequired ? (
              ...
              <div className="flex items-center gap-2 p-2 rounded-xl bg-gray-50"><span className="w-2 h-2 rounded-full bg-gray-400" /> Garant principal — <span className="text-zinc-500">Optionnel</span></div>
            )}
