    ...
    <section className="py-14 bg-white">
      <div className="max-w-[1400px] mx-auto px-4 lg:px-6">
        <div className="flex items-end justify-between mb-8">
          <h2 className="text-[26px] lg:text-[32px] font-extrabold">Services populaires 🔥</h2>
          <a className="text-sm font-bold text-[#FF6B35] hover:underline">Voir tout →</a>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {services.map((s,i)=>(
            <div key={i} className="group relative rounded-[18px] overflow-hidden h-[180px] cursor-pointer">
              ...
              <div className="relative p-4 h-full flex flex-col justify-between text-white">
                ...
                <p className="font-bold text-[15px] leading-tight">{s.title}</p>
              </div>
            </div>
          ))}
        </div>
